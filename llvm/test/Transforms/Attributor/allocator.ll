...
; RUN: opt -aa-pipeline=basic-aa -debug-only=attributor -passes=attributor -attributor-manifest-internal -attributor-annotate-decl-cs  -S < %s | FileCheck %s --check-prefixes=CHECK,TUNIT
; RUN: opt -aa-pipeline=basic-aa -debug-only=attributor -passes=attributor-cgscc -attributor-manifest-internal -attributor-annotate-decl-cs -S < %s | FileCheck %s --check-prefixes=CHECK,CGSCC
...
; CHECK-NEXT:    [[MALLOCCALL:%.*]] = tail call ptr @malloc(i32 4)
; CHECK-NEXT:    store ptr [[MALLOCCALL]], ptr [[F]], align 8
; CHECK-NEXT:    [[TMP0:%.*]] = load i32, ptr [[VAL]], align 4
; CHECK-NEXT:    [[ADD:%.*]] = add nsw i32 [[TMP0]], 10
; CHECK-NEXT:    store i32 [[ADD]], ptr [[MALLOCCALL]], align 4
; CHECK-NEXT:    [[TMP1:%.*]] = load i32, ptr [[MALLOCCALL]], align 4
...
; CHECK-NEXT:    store ptr [[MALLOCCALL]], ptr [[F]], align 8
; CHECK-NEXT:    [[TMP0:%.*]] = load i32, ptr [[VAL]], align 4
; CHECK-NEXT:    store i32 [[TMP0]], ptr [[MALLOCCALL]], align 4
; CHECK-NEXT:    [[TMP1:%.*]] = load i32, ptr [[MALLOCCALL]], align 4
; CHECK-NEXT:    [[CALL2:%.*]] = call i32 (ptr, ...) @printf(ptr noundef nonnull dereferenceable(17) @.str, i32 noundef [[TMP1]])
; CHECK-NEXT:    ret void
;
...
; CHECK-SAME: (i32 noundef [[VAL:%.*]]) #[[ATTR1:[0-9]+]] {
; CHECK-NEXT:  entry:
...
attributes #8 = { nounwind allocsize(0) allockind("alloc,uninitialized") "alloc-family"="malloc"}

; Function Attrs: nounwind allocsize(0)
declare noalias ptr @malloc(i64) #8
;.
; CHECK: attributes #[[ATTR0]] = { nounwind }
; CHECK: attributes #[[ATTR1]] = { mustprogress nofree norecurse nosync nounwind willreturn memory(none) }
; CHECK: attributes #[[ATTR2:[0-9]+]] = { nounwind allockind("alloc,uninitialized") allocsize(0) "alloc-family"="malloc" }
;.
